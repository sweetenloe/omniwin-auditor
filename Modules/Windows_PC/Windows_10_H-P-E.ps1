...
Function Reverse-SID ($sidString) {
 ...
 $sidString = $sidString -creplace '^[^\\]*=', ''
 $sidString = $sidString.replace("*", "")
 $sidString = $sidString.replace(" ", "")
 if ( $sidString -ne $null){
 $array = @()
 $array = $sidString.Split(",") 
...
$fldrname = "Audit_CONF_" + $OSName + "_" + $date
...
$shareFileName = "./SHARE " + "$OSName" + ".csv"
...
foreach ( $share in $listShare) {
 ...
 $rights = Get-SmbShareAccess $share.name
 
 
 foreach ( $right in $rights) {
 
 
  $shareArray += addShare -NS $share.name -CS $share.path -US $right.AccountName -TS $right.AccessControlType -NDS $right.AccessRight
 
 
 }
}

$shareArray | ConvertTo-CSV -NoTypeInformation -Delimiter ";" | Set-Content $shareFileName

Write-Host "#########>Take Appdata Information<#########" -ForegroundColor DarkGreen
$profilePath = (Get-ItemProperty -Path HKLM:\SOFTWARE\Microsoft\Windows' 'NT\CurrentVersion\ProfileList\).ProfilesDirectory
  
  
$profilePresent = Get-ChildItem $profilePath 
  
$appResult = @()
$appFileName = "./APPDATA" + "$OSName" + ".csv"
  
  
foreach ( $profile in $profilePresent) {
  
  $verifyAppdata = Test-Path $profilePath\$profile\Appdata
  
  if ($verifyAppdata -eq $true) {
  
  $resultFiles = Get-ChildItem $profilePath\$profile\Appdata -Recurse -Include *.bat, *.exe, *.ps1, *.ps1xml, *.PS2, *.PS2XML, *.psc1, *.PSC2, *.msi, *.py, *.pif, *.MSP , *.COM, *.SCR, *.hta, *.CPL, *.MSC, *.JAR, *.VB, *.VBS, *.VBE, *.JS, *.JSE, *.WS, *.wsf, *.wsc, *.wsh, *.msh, *.MSH1, *.MSH2, *.MSHXML, *.MSH1XML, *.MSH2XML, *.scf, *.REG, *.INF   | Select-Object Name, Directory, Fullname 
  
  foreach ($riskyfile in $resultFiles) {
...
$fnameFeature = "./Feature-" + "$OSName" + ".txt"
$fnameOptionnalFeature = "./OptionnalFeature-" + "$OSName" + ".txt" 
if ( $OSversion -match "Server") {
 import-module servermanager
 
 Get-WindowsFeature | where-object {$_.Installed -eq $True} |Format-Table * -Autosize >> ./$fnameFeature 
 
}
Get-WindowsOptionalFeature -Online | where-object {$_.State -eq "Enabled"} |Format-Table * -Autosize >> $fnameOptionnalFeature
...
"LocalAddress;LocalPort;State;OwningProcess;Path" > $fnamePort
...
$fnameStartup = "./Startup- " + "$OSName" + ".txt"
"HKCU:\Software\Microsoft\Windows\CurrentVersion\Run" >> $fnameStartup
Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run" |Select-Object * -exclude PSPath,PSParentPath, PSChildName, PSProvider, PSDrive >> $fnameStartup
"HKCU:\Software\Microsoft\Windows\CurrentVersion\RunOnce" >> $fnameStartup
Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\RunOnce" |Select-Object * -exclude PSPath,PSParentPath, PSChildName, PSProvider, PSDrive >> $fnameStartup
"HKCU:\Software\Microsoft\Windows NT\CurrentVersion\Windows" >> $fnameStartup
Get-ItemProperty "HKCU:\Software\Microsoft\Windows NT\CurrentVersion\Windows" |Select-Object * -exclude PSPath,PSParentPath, PSChildName, PSProvider, PSDrive >> $fnameStartup
"HKLM:\Software\Microsoft\Windows\CurrentVersion\Run" >> $fnameStartup
Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\Run" |Select-Object * -exclude PSPath,PSParentPath, PSChildName, PSProvider, PSDrive >> $fnameStartup
"HKLM:\Software\Microsoft\Windows\CurrentVersion\RunOnce" >> $fnameStartup
Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\RunOnce" |Select-Object * -exclude PSPath,PSParentPath, PSChildName, PSProvider, PSDrive >> $fnameStartup
...
$traitement = Get-Content $fnameNetAccount |Select-String -pattern '(DurA''AAe du verrouillage)|(Lockout duration)'
...
$traitement = Get-Content $fnameNetAccount |Select-String -pattern '(Seuil de verrouillage)|(Lockout threshold)'
...
$traitement = Get-Content $fnameNetAccount |Select-String -pattern "(FenA'AAatre d'observation du verrouillage)|(Lockout observation window)"
...
$traitement = Get-Content $auditconfigfile |Select-String -pattern "(Autres A'AAvA'AAnements d'ouverture de session)|(Other Account Management Events)"
$traitement = $traitement.line
$chaine += $traitement
$chaine>> $fname
...
$traitement = Get-Content $auditconfigfile |Select-String -pattern "(Gestion des groupes de sA'AAcuritA'AA)|(Security Group Management)"
$traitement = $traitement.line
...
$traitement = Get-Content $auditconfigfile |Select-String -pattern "(A'AAvA'AAnements Plug-and-Play)|(PNP Activity)"
$traitement = $traitement.line
...
$traitement = Get-Content $auditconfigfile |Select-String -pattern "(CrA'AAation du processus)|(Process Creation)"
$traitement = $traitement.line
...
$traitement = Get-Content $auditconfigfile |Select-String -pattern "(Appartenance A'AA un groupe)|(Group Membership)"
$traitement = $traitement.line
$chaine += $traitement
$chaine>> $fname
...
$traitement = Get-Content $auditconfigfile |Select-String -pattern "(Autres A'AAvA'AAnements d'ouverture/fermeture de session)|(Other Logon/Logoff Events)"
$traitement = $traitement.line 
...
$traitement = Get-Content $auditconfigfile |Select-String -pattern "(Ouverture de session spA'AAciale)|(Special Logon)"
$traitement = $traitement.line 
...
$traitement = Get-Content $auditconfigfile |Select-String -pattern "(Autres A'AAvA'AAnements d'accA''AA s A''AA  l'objet)|(Other Object Access Event)"
$traitement = $traitement.line 
...
$traitement = Get-Content $auditconfigfile |Select-String -pattern "(Modification de la stratA'AAgie d'audit)|(Audit Policy Change)"
$traitement = $traitement.line 
...
$traitement = Get-Content $auditconfigfile |Select-String -pattern "(Modification de la stratA'AAgie d'authentification)|(Authentication Policy Change)"
$traitement = $traitement.line 
...
$traitement = Get-Content $auditconfigfile |Select-String -pattern "(Modification de la stratA'AAgie d'autorisation)|(Authorization Policy Change)"
$traitement = $traitement.line 
...
$traitement = Get-Content $auditconfigfile |Select-String -pattern "(Modification de la stratA'AAgie de niveau rA'AA gle MPSSVC)|(MPSSVC Rule-Level Policy Change)"
$traitement = $traitement.line 
...
$traitement = Get-Content $auditconfigfile |Select-String -pattern "(Utilisation de privilA'AAges sensibles)|(Sensitive Privilege Use)"
$traitement = $traitement.line 
...
$traitement = Get-Content $auditconfigfile |Select-String -pattern "(Autres A'AAvA'AAnements systA'AAme)|(Other System Events)"
$traitement = $traitement.line
...
$traitement = Get-Content $auditconfigfile |Select-String -pattern "(Modification de l'A'AAtat de la sA'AAcuritA'AA)|(Security State Change)"
$traitement = $traitement.line 
...
$traitement = Get-Content $auditconfigfile |Select-String -pattern "(Extension systA'AAme de sA'AAcuritA'AA)|(Security System Extension)"
$traitement = $traitement.line 
...
$traitement = Get-Content $auditconfigfile |Select-String -pattern "(IntA'AAgritA'AA du systA'AAme)|(System Integrity)"
$traitement = $traitement.line 
...
$traitement = $null
$id = "WU" + "$indextest"
$chaine = "$id" + ";" + "(L1)Ensure 'Remove access to AAAasAA...aPause updatesAAAasAAA feature' is set to 'Enabled', value must be 1" + ";"
$exist = Test-Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate"
if ( $exist -eq $true) {
